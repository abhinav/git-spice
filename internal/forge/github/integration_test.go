...
func TestIntegration_Repository(t *testing.T) {
	...
	_, err := github.NewRepository(t.Context(), new(github.Forge), "abhinav", "git-spice", logtest.New(t), ghc, nil)
	require.NoError(t, err)
}
...
func TestIntegration_Repository_FindChangeByID(t *testing.T) {
	rec := newRecorder(t, t.Name())
	ghc := newGitHubClient(rec.GetDefaultClient())
	repo, err := github.NewRepository(t.Context(), new(github.Forge), "abhinav", "git-spice", logtest.New(t), ghc, _gitSpiceRepoID)
	require.NoError(t, err)
...
func TestIntegration_Repository_ListChangeTemplates(t *testing.T) {
	...
	t.Run("present", func(t *testing.T) {
		rec := newRecorder(t, t.Name())
		ghc := newGitHubClient(rec.GetDefaultClient())
		repo, err := github.NewRepository(t.Context(), new(github.Forge), "golang", "go", logtest.New(t), ghc, nil)
		require.NoError(t, err)
...
func TestIntegration_Repository_SubmitEditChange(t *testing.T) {
	...
	repo, err := github.NewRepository(
		t.Context(), new(github.Forge), "abhinav", "test-repo", logtest.New(t), ghc, _testRepoID,
	)
	require.NoError(t, err)
