...
func TestWorktree_RestoreIntegration(t *testing.T) {
	...
	t.Run("RestoreOneFile", func(t *testing.T) {
		...
		// Verify file1.txt was restored but file2.txt remains modified
		file1, err := os.ReadFile(fixture.Dir() + "/file1.txt")
		require.NoError(t, err)
		assert.Equal(t, "original content 1\n", string(file1))

		file2, err := os.ReadFile(fixture.Dir() + "/file2.txt")
		require.NoError(t, err)
		assert.Equal(t, "modified content 2\n", string(file2))
	})
	...
	t.Run("RestoreAllFiles", func(t *testing.T) {
		...
		body, err := os.ReadFile(fixture.Dir() + "/original.txt")
		require.NoError(t, err)
		assert.Equal(t, "original content\n", string(body))
		assert.FileExists(t, fixture.Dir()+"/new.txt")
	})
