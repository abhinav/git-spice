...
func TestStagedComments(t *testing.T) {
	...
	t.Run("SaveAndLoad", func(t *testing.T) {
		comments := &state.StagedComments{
			...
			Comments: []state.StagedComment{
				...
				},
				{
					LocalID: 2,
					File:    "handler.go",
					Lines:   forge.InlineCommentLine(15),
		...
		assert.Equal(t, forge.InlineCommentLine(42), got.Comments[0].Lines)
		assert.Equal(t, forge.InlineCommentThreadID("thread-abc"), got.Comments[1].ID)
	})
			...
			Comments: []state.StagedComment{
				{
					LocalID: 1,
					File:    "new.go",
					Lines:   forge.InlineCommentLine(1),
					Body:    "New comment",
				},
			},
		}
			...
			Comments: []state.StagedComment{
				{
					LocalID: 1,
					File:    "b.go",
					Lines:   forge.InlineCommentLine(2),
					Body:    "B",
				},
			},
		}
