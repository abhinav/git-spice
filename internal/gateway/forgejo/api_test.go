...
func TestClient_LabelCreate(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, http.MethodPost, r.Method)
		assert.Equal(t, "/api/v1/repos/owner/repo/labels", r.URL.Path)
		assertJSONBody(t, r, `{
			"name":"blocked",
			"color":"ededed"
		}`)
	...
	client := newTestClient(t, srv)
	label, _, err := client.LabelCreate(
		t.Context(),
		"owner",
		"repo",
		&CreateLabelOption{
	...
	assert.Equal(t, "blocked", label.Name)
}

func TestClient_ContentsList(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, http.MethodGet, r.Method)
		assert.Equal(
			t,
			"/api/v1/repos/owner/repo/contents/.forgejo/PULL_REQUEST_TEMPLATE",
			r.URL.Path,
		)
		writeJSON(t, w, http.StatusOK, []*ContentsResponse{
			{
				Name: "default.md",
				Path: ".forgejo/PULL_REQUEST_TEMPLATE/default.md",
				Type: "file",
			},
		})
	}))
	...
	client := newTestClient(t, srv)
	contents, _, err := client.ContentsList(
		t.Context(),
		"owner",
		"repo",
	...
	assert.Equal(t, "default.md", contents[0].Name)
}
