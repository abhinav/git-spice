...
func (cmd *branchCommentEditCmd) editForge(
	...
	editor, ok := forgeRepo.(forge.ReviewCommentEditor)
	if !ok {
		return errors.New(
			"forge does not support review comment editing",
		)
	}

	// Recover the provider-owned ID and current body from the listed thread.
	var target *forge.ReviewComment
	for thread, err := range reviewRepo.ListReviewThreads(
	...
	if err := editor.UpdateReviewComment(
		ctx, target.ID, body,
	); err != nil {
